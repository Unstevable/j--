...
abstract class JComparisonExpression extends JBooleanBinaryExpression {
    ...
    public JExpression analyze(Context context) {
        ...
        lhs.type().mustMatchOneOf(line(), Type.INT, Type.LONG, Type.DOUBLE); // Added for project5: Longs and Doubles
...
class JGreaterEqualOp extends JComparisonExpression {
    ...
    /** Added codegen for project5
     * {@inheritDoc}
     */
    public void codegen(CLEmitter output, String targetLabel, boolean onTrue) {
        lhs.codegen(output);
        rhs.codegen(output);
        // If onTrue, we do the >= comparison; otherwise, the complement, which is <
...
class JLessThanOp extends JComparisonExpression {
    ...
    public void codegen(CLEmitter output, String targetLabel, boolean onTrue) {
        lhs.codegen(output);
        rhs.codegen(output);
        // If onTrue, then we do <; otherwise the complement, which is >=
