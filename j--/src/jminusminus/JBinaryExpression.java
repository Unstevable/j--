...
class JMultiplyOp extends JBinaryExpression {
    ...
    /** Added analyze and codegen for longs and doubles for project5
     * {@inheritDoc}
     */
    public JExpression analyze(Context context) {
        ...
        if (lhs.type() == Type.INT && rhs.type() == Type.INT) {
            ...
            type = Type.LONG; // Multiplying longs
        } else if (lhs.type() == Type.DOUBLE && rhs.type() == Type.DOUBLE){
            type = Type.DOUBLE; // Multiplying doubles
        } else {
            type = Type.ANY;
    ...
    public void codegen(CLEmitter output) {
        ...
        rhs.codegen(output);
        if (lhs.type() == Type.INT && rhs.type() == Type.INT){
            output.addNoArgInstruction(IMUL); // Multiplying ints
        } else if (lhs.type() == Type.LONG && rhs.type() == Type.LONG){
            output.addNoArgInstruction(LMUL); // Multiplying longs
        } else if (lhs.type() == Type.DOUBLE && rhs.type() == Type.DOUBLE){
            output.addNoArgInstruction(DMUL); // Multiplying doubles
        }
    }
}
...
class JPlusOp extends JBinaryExpression {
    ...
    /** Added long and double functionality to analyze/codegen for project5
     * {@inheritDoc}
     */
    public JExpression analyze(Context context) {
        lhs = (JExpression) lhs.analyze(context);
        ...
        if (lhs.type() == Type.STRING || rhs.type() == Type.STRING) {
            ...
            type = Type.LONG; // Adding longs
        } else if (lhs.type() == Type.DOUBLE && rhs.type() == Type.DOUBLE){
            type = Type.DOUBLE; // Adding doubles
        } else {
            type = Type.ANY;
            JAST.compilationUnit.reportSemanticError(line(), "Invalid operand types for +");
        }
    ...
    public void codegen(CLEmitter output) {
        ...
        rhs.codegen(output);
        if (lhs.type() == Type.INT && rhs.type() == Type.INT){
            output.addNoArgInstruction(IADD); // Adding ints
        } else if (lhs.type() == Type.LONG && rhs.type() == Type.LONG){
            output.addNoArgInstruction(LADD); // Adding longs
        } else if (lhs.type() == Type.DOUBLE && rhs.type() == Type.DOUBLE){
            output.addNoArgInstruction(DADD); // Adding doubles
        }
    }
}
...
class JSubtractOp extends JBinaryExpression {
    ...
    public JExpression analyze(Context context) {
        ...
        if (lhs.type() == Type.INT && rhs.type() == Type.INT) {
            ...
            type = Type.LONG; // Subtracting longs
        } else if (lhs.type() == Type.DOUBLE && rhs.type() == Type.DOUBLE){
            type = Type.DOUBLE; // Subtracting doubles
        } else {
            type = Type.ANY;
            JAST.compilationUnit.reportSemanticError(line(), "Invalid operand types for -");
        }
    ...
    public void codegen(CLEmitter output) {
        ...
        rhs.codegen(output);
        if (lhs.type() == Type.INT && rhs.type() == Type.INT){
            output.addNoArgInstruction(ISUB); // Subtracting ints
        } else if (lhs.type() == Type.LONG && rhs.type() == Type.LONG){
            output.addNoArgInstruction(LSUB); // Subtracting longs
        } else if (lhs.type() == Type.DOUBLE && rhs.type() == Type.DOUBLE){
            output.addNoArgInstruction(DSUB); // Subtracting doubles
        }
    }
}
...
class JDivideOp extends JBinaryExpression {
     ...
     * {@inheritDoc}
     */
    // Adding analyze for division operations
    public JExpression analyze(Context context) {
        lhs = (JExpression) lhs.analyze(context);
        rhs = (JExpression) rhs.analyze(context);
        lhs.type().mustMatchOneOf(line(), Type.INT, Type.LONG, Type.DOUBLE);
        ...
        if (lhs.type() == Type.INT && rhs.type() == Type.INT) {
            ...
            type = Type.LONG; // Dividing longs
        } else if (lhs.type() == Type.DOUBLE && rhs.type() == Type.DOUBLE){
            type = Type.DOUBLE; // Dividing doubles
        } else {
            type = Type.ANY;
            JAST.compilationUnit.reportSemanticError(line(), "Invalid operand types for /");
        }
     ...
     * {@inheritDoc}
     */
    // Adding codegen for division operations
    public void codegen(CLEmitter output) {
        lhs.codegen(output);
        rhs.codegen(output);
        if (lhs.type() == Type.INT && rhs.type() == Type.INT){
            output.addNoArgInstruction(IDIV); // Dividing ints
        } else if (lhs.type() == Type.LONG && rhs.type() == Type.LONG){
            output.addNoArgInstruction(LDIV); // Dividing longs
        } else if (lhs.type() == Type.DOUBLE && rhs.type() == Type.DOUBLE){
            output.addNoArgInstruction(DDIV); // Dividing doubles
        }
    }
}
...
class JRemainderOp extends JBinaryExpression {
     ...
     * {@inheritDoc}
     */
    // Add analyze for modulo operations
    public JExpression analyze(Context context) {
        lhs = (JExpression) lhs.analyze(context);
        rhs = (JExpression) rhs.analyze(context);
        lhs.type().mustMatchOneOf(line(), Type.INT, Type.LONG, Type.DOUBLE);
        ...
        if (lhs.type() == Type.INT && rhs.type() == Type.INT) {
            ...
            type = Type.LONG; // Modulo longs
        } else if (lhs.type() == Type.DOUBLE && rhs.type() == Type.DOUBLE) {
            type = Type.DOUBLE; // Modulo doubles
        } else {
            type = Type.ANY;
            JAST.compilationUnit.reportSemanticError(line(), "Invalid operand types for %");
        }
     ...
     * {@inheritDoc}
     */
    // Add codegen for modulo operations
    public void codegen(CLEmitter output) {
        lhs.codegen(output);
        rhs.codegen(output);
        if (lhs.type() == Type.INT && rhs.type() == Type.INT){
            output.addNoArgInstruction(IREM); // Modulo ints
        } else if (lhs.type() == Type.LONG && rhs.type() == Type.LONG){
            output.addNoArgInstruction(LREM); // Modulo longs
        } else if (lhs.type() == Type.DOUBLE && rhs.type() == Type.DOUBLE){
            output.addNoArgInstruction(DREM); // Modulo doubles
        }
    }
}
